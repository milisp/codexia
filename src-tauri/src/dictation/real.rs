...
/// Inner mutable state, guarded by a single async mutex on [`DictationState`].
struct DictationInner {
    ...
    session: Option<DictationSessionHandle>,
    processing_cancel: Option<Arc<AtomicBool>>,
    cached_context: Option<CachedWhisperContext>,
    /// Model id resolved from app settings; callers pass `None` to use this default.
...
/// Managed Tauri state for on-device dictation (mic capture + Whisper transcription).
...
impl DictationState {
    ...
    /// Overrides the default model id used when a command is called with `model_id: None`.
    /// Call this once at startup from app settings if desired; safe to skip (defaults to "base").
    pub async fn set_default_model_id(&self, model_id: String) {
        let mut inner = self.inner.lock().await;
        inner.default_model_id = model_id;
    }
}
...
async fn clear_processing_cancel(state: &DictationState, cancel_flag: &Arc<AtomicBool>) -> bool {
...
async fn update_status(app: &AppHandle, state: &DictationState, status: DictationModelStatus) {
    {
        let mut inner = state.inner.lock().await;
...
async fn clear_download_state(state: &DictationState) {
    let mut inner = state.inner.lock().await;
    inner.download_cancel = None;
    inner.download_task = None;
}

async fn resolve_model_id(state: &DictationState, model_id: Option<String>) -> String {
    let candidate = if let Some(model_id) = model_id {
        model_id
    } else {
        let inner = state.inner.lock().await;
        if inner.default_model_id.trim().is_empty() {
            DEFAULT_MODEL_ID.to_string()
        } else {
            inner.default_model_id.clone()
        }
    };
...
async fn refresh_status(
    ...
    let mut inner = state.inner.lock().await;
    if inner.model_status.state == DictationModelState::Downloading
        && inner.model_status.model_id == model_id
    {
        return inner.model_status.clone();
    }

    let path = match model_path(app, model_id) {
        Ok(path) => path,
        Err(error) => {
            inner.model_status = DictationModelStatus {
    ...
    // A merely-existing file isn't enough: a failed/canceled download or an
    // interrupted write can leave a zero-byte or truncated file behind.
    // Treat anything implausibly small as missing rather than ready.
    let is_valid = std::fs::metadata(&path)
        .map(|metadata| metadata.len() > 0)
        .unwrap_or(false);

    if is_valid {
        inner.model_status = ready_status(model_id, &path);
    } else {
        inner.model_status = missing_status(model_id);
    }
    inner.model_status.clone()
}
...
pub async fn dictation_download_model(
    ...
    let cancel_flag = Arc::new(AtomicBool::new(false));
    {
        ...
        if inner.model_status.state == DictationModelState::Downloading
            && inner.model_status.model_id != model_id
        {
            if let Some(flag) = inner.download_cancel.take() {
        ...
        inner.download_cancel = Some(cancel_flag.clone());
        inner.model_status = DictationModelStatus {
    ...
    let task = tokio::spawn(async move {
        ...
        {
            let mut inner = state.inner.lock().await;
            inner.default_model_id = model_id_clone.clone();
        }
        update_status(&app_handle, &state, status).await;
        clear_download_state(&state).await;
    });
...
pub async fn dictation_start(
    ...
    {
        let inner = state.inner.lock().await;
        if inner.session_state != DictationSessionState::Idle {
    ...
    {
        let mut inner = state.inner.lock().await;
        inner.session_state = DictationSessionState::Listening;
        inner.session = Some(DictationSessionHandle {
            stop: stop_tx,
            stopped: stopped_rx,
            audio,
            sample_rate,
...
pub async fn dictation_request_permission(app: AppHandle) -> Result<bool, String> {
...
pub async fn dictation_stop(
    ...
    let (audio, sample_rate, model_id, preferred_language, stopped, stop_tx) = {
        let mut inner = state.inner.lock().await;
        if inner.session_state != DictationSessionState::Listening {
        ...
        inner.session_state = DictationSessionState::Processing;
        inner.processing_cancel = Some(Arc::clone(&cancel_flag));
        let session = inner
            .session
            .take()
    ...
    tokio::spawn(async move {
        ...
        if cancel_flag.load(Ordering::Relaxed) {
            let state_handle = app_handle.state::<DictationState>();
            clear_processing_cancel(&state_handle, &cancel_flag).await;
            return;
        }

        let state_handle = app_handle.state::<DictationState>();
        let cached_context = {
            let inner = state_handle.inner.lock().await;
            inner
                .cached_context
                .as_ref()
        ...
        let context = if let Some(context) = cached_context {
            ...
            let model_path = match model_path(&app_handle, &model_id) {
                ...
                Err(error) => {
                    ...
                    let mut inner = state_handle.inner.lock().await;
                    inner.session_state = DictationSessionState::Idle;
                    emit_event(
                        &app_handle,
                        DictationEvent::State {
            ...
            let mut inner = state_handle.inner.lock().await;
            inner.cached_context = Some(CachedWhisperContext {
                model_id: model_id.clone(),
...
pub async fn dictation_cancel(
    ...
    {
        let mut inner = state.inner.lock().await;
        if inner.session_state == DictationSessionState::Processing {
            if let Some(flag) = inner.processing_cancel.take() {
                flag.store(true, Ordering::Relaxed);
            }
            inner.session_state = DictationSessionState::Idle;
            emit_event(
...
fn start_capture_thread(
    ...
    let err_fn = move |error| {
        ...
        tauri::async_runtime::spawn(async move {
            let state_handle = state_app.state::<DictationState>();
            let should_emit = {
                let mut inner = state_handle.inner.lock().await;
                if inner.session_state == DictationSessionState::Idle {
                    false
